...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="slider-row">
      <label for="ditherStrength">抖动强度 <span id="ditherStrengthVal">1.0</span></label>
      <input type="range" id="ditherStrength" min="0" max="5" step="0.1" value="1.0">
    </div>
    <div class="slider-row">
      <label for="contrast">对比度 <span id="contrastVal">1.2</span></label>
...
<script>
        ...
        document.getElementById('ditherStrength').addEventListener('input', function () {
            document.getElementById('ditherStrengthVal').textContent =
    ...
    return imageData;
        ...
        async function waitForFlushDone() {
            ...
            while (Date.now() - started < 60000) {
                ...
                let j;
                try {
                    j = await (await fetch('/upload/status')).json();
                } catch (e) {
                    continue;
                }
                if (j.state === 'busy') {
                    ...
                    setTip('墨屏刷屏中…约需 15–20 秒（已 ' + sec + 's）');
                    continue;
                }
                if (j.state === 'done') {
                    setTip('刷屏完成，请看相框画面', 'ok');
                    return;
                }
                if (j.state === 'fail') {
                    setTip('刷屏失败或已取消，可再试一次', 'err');
                    return;
                }
                // idle：上传刚结束可能尚未进入 busy，稍等；已见过 busy 再 idle 也当完成
                if (sawBusy || Date.now() - started > 3000) {
                    setTip('刷屏完成，请看相框画面', 'ok');
                    return;
                }
            }
            setTip('等待刷屏超时，请查看相框是否已更新', 'err');
        }
