...
| 按键 | GPIO | 简要作用 |
| --- | --- | --- |
| 模式键 | **4** | 双击：局域网传图 ↔ 小程序推送（只改灯并连闪 3 次；单击无效） |
...
> **LED**：下载/刷屏为**紫色呼吸**；系统键按住：0–3s 橙常亮 → 3–6s 黄慢闪（松手即重配）→ 6–8s 红快闪（即将出厂）；取消请求黄闪两下（全刷波形仍无法中断）；断网黄闪并重连，约 5 分钟仍失败则清 WiFi 进配网；局域网模式停留约 10 分钟青灯连闪提醒切回推送。  
...
4. **青灯常亮**：双击模式键进入局域网传图（灯连闪）；长按执行键约 2 秒刷出 `http://<IP>/upload`，同 WiFi 浏览器打开即可传图（上传后页面会轮询刷屏进度）。
